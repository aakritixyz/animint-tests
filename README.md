# Animint Projects – Consolidated

This repository contains all my Animint projects, organized by difficulty. Click the links to view the live demos or the source code on GitHub.

---

## Easy
- **Ten Points Test**  
  - Demo: [Live](https://aakritixyz.github.io/ten-points-animint/)  
  - Source Code: [GitHub](https://github.com/aakritixyz/ten-points-animint)

---

## Medium
- **K-Means Visualization**  
  - Demo: [Live](https://aakritixyz.github.io/kmeans-animint-test/)  
  - Source Code: [GitHub](https://github.com/aakritixyz/kmeans-animint-test)

- **WorldBank Visualisation**  
  - Demo: [Live](https://aakritixyz.github.io/worldbank-animint-test/)  
  - Source Code: [GitHub](https://github.com/aakritixyz/worldbank-animint-test)
 
- **Monte Carlo Sample Mean
  - Demo: [Live](https://aakritixyz.github.io/mc-sample-mean-test/)
  - Source Code: [GitHub]([mc-sample-mean-test/](https://github.com/aakritixyz/mc-sample-mean-test))

---
